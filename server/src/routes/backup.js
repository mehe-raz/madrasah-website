...
// GENERATED BY DEFAULT AS IDENTITY columns (every restored table here
// except "settings", which has a text key, not a numeric id) keep their own
// sequence, separate from the actual id values just inserted. Restoring
// explicit ids — so students/attendance/payments/hifz_logs etc. keep the
// exact ids their cross-references (attendance.studentId, income.studentId,
// hifz_logs.studentId...) already point at — never advances that sequence;
// Postgres has no way to know we just did that. Left alone, the very next
// INSERT that doesn't specify an id (i.e. every normal "add a new
// student/attendance/income/expense/hifz entry" from the app, from then on)
// calls nextval(), which still returns 1 and collides with the id=1 row
// that was just restored — this is the "duplicate" error reported right
// after restoring a backup that has real data in it. This bumps each
// table's sequence past the highest id actually restored so nextval() is
// safe again. `table` always comes from the BACKUP_TABLES whitelist (same
// trust level as the TRUNCATE TABLE ${table} call above), never user input.
// GENERATED BY DEFAULT AS IDENTITY columns (every restored table here
// except "settings", which has a text key, not a numeric id) keep their own
// sequence, separate from the actual id values just inserted. Restoring
// explicit ids — so students/attendance/payments/hifz_logs etc. keep the
...
// trust level as the TRUNCATE TABLE ${table} call above), never user input.
//
// `hasIdColumn` must be checked BEFORE calling pg_get_serial_sequence:
// unlike most "return NULL if not applicable" Postgres functions, it
// throws ("column ... of relation ... does not exist") rather than
// returning NULL when the column itself doesn't exist — which "settings"
// hits every time, since it has no id column at all. allowedColumns is
// already fetched by the caller, so reuse it instead of a second query.
async function resetIdentitySequence(tx, table, allowedColumns) {
  if (!allowedColumns.includes("id")) return;
  await tx.run(`
    DO $$
    ...
    BEGIN
      ...
      IF seqname IS NOT NULL THEN
        EXECUTE format('SELECT setval(%L, COALESCE((SELECT MAX(id) FROM ${table}), 0) + 1, false)', seqname);
      END IF;
    END $$;
  `);
}
...
async function restoreJsonBackup(data, options = {}) {
  ...
  return await db.withTransaction(async (tx) => {
    ...
    for (const table of tablesToRestore) {
      ...
      await resetIdentitySequence(tx, table, allowedColumns);
