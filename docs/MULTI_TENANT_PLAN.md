...
| ৬ | **Billing + Migration Tooling** | সাবস্ক্রিপশন/পেমেন্ট, মেয়াদ শেষে অটো-সাসপেন্ড, সব tenant schema-তে একসাথে migration চালানোর স্ক্রিপ্ট | ✅ সম্পন্ন (এই ধাপ) |
...
লাগবে), আর সব tenant schema-তে একসাথে migration চালানোর টুল — এটাই ভাগ ৬ (নিচে দেখুন)।

---

## ভাগ ৬ — Billing + Migration Tooling (এই ধাপে যা হয়েছে)

### স্কোপ সংক্রান্ত একটা গুরুত্বপূর্ণ সিদ্ধান্ত

bKash/Nagad/SSLCommerz-এর মতো গেটওয়ের *লাইভ* API ইন্টিগ্রেশনের জন্য একটা রেজিস্টার্ড মার্চেন্ট
অ্যাকাউন্ট আর পাবলিক HTTPS কলব্যাক URL লাগে — এই দুটোর কোনোটাই এখনো এই প্রজেক্টের নেই, আর
sandbox পরিবেশে নেটওয়ার্ক/npm অ্যাক্সেসও নেই। তাই ভাগ ৬ বাস্তবসম্মতভাবে একটা **ম্যানুয়ালি
নিশ্চিত করা পেমেন্ট লেজার** বানিয়েছে: আপনি (প্ল্যাটফর্ম অপারেটর) bKash/Nagad/ব্যাংকে টাকা পাওয়া
নিশ্চিত হওয়ার পর প্যানেল বা CLI দিয়ে সেটা রেকর্ড করবেন, আর সিস্টেম স্বয়ংক্রিয়ভাবে সাবস্ক্রিপশনের
মেয়াদ বাড়িয়ে দেবে/অ্যাকাউন্ট সক্রিয় করে দেবে। ভবিষ্যতে সত্যিকারের গেটওয়ে বসাতে চাইলে সেই
গেটওয়ের webhook থেকে ঠিক এই একই `registryDb.recordPayment()` ফাংশন কল করলেই হবে — বাকি
কোনো কিছু বদলাতে হবে না।

### নতুন ফাইল

- `server/src/migrateTenants.js` — `migrateAllTenants(sqlText)`: দেওয়া SQL প্রতিটা
  `tenant_xxx` schema-তে আলাদা আলাদা ট্রানজ্যাকশনে চালায় — একটাতে ব্যর্থ হলে বাকিগুলোর জন্য
  চেষ্টা থামে না, প্রতিটার ফলাফল (সফল/ব্যর্থ + এরর মেসেজ) আলাদাভাবে রিপোর্ট করা হয়। এটাই এখন
  থেকে নতুন কলাম/টেবিল-পরিবর্তন সব প্রতিষ্ঠানে ছড়িয়ে দেওয়ার একমাত্র উপায় — আগে শুধু `public`
  schema-র জন্য `db.js`-এর `initSchema()` চলত।
- `server/src/billing.js` — `startExpiryScanJob()`: একটা প্লেইন `setInterval` (নতুন কোনো cron
  প্যাকেজ ছাড়াই — AGENTS.md-এর নিয়ম অনুযায়ী নতুন dependency এড়ানো হয়েছে) যেটা নির্দিষ্ট বিরতিতে
  (ডিফল্ট ১ ঘণ্টা) `registryDb.runExpiryScan()` চালায়।

### পরিবর্তিত ফাইল

- `server/sql/registry_schema.sql` — নতুন `registry.payments` টেবিল (পেমেন্টের পরিমাণ, মাধ্যম,
  রেফারেন্স/ট্রানজেকশন আইডি, কোন তারিখ পর্যন্ত মেয়াদ বাড়লো)।
- `server/src/registryDb.js` — তিনটা নতুন ফাংশন:
  - `recordPayment(institutionId, {...})` — একটা ট্রানজ্যাকশনে পেমেন্ট রেকর্ড করে + বিদ্যমান
    মেয়াদের উপর নতুন মেয়াদ যোগ করে (আগেভাগে পেমেন্ট করলে কোনো দিন নষ্ট হয় না) + status
    স্বয়ংক্রিয়ভাবে 'active' করে দেয়।
  - `listPayments()` — নির্দিষ্ট প্রতিষ্ঠানের বা সব প্রতিষ্ঠানের পেমেন্ট-ইতিহাস।
  - `runExpiryScan()` — যাদের ট্রায়াল/সাবস্ক্রিপশনের মেয়াদ পার হয়ে গেছে তাদের 'suspended' করে
    দেয়, প্রতিটার জন্য অডিট-লগে `system:expiry-scan` actor দিয়ে এন্ট্রি রাখে (ম্যানুয়াল
    সাসপেনশন থেকে স্পষ্টভাবে আলাদা দেখানোর জন্য)।
- `server/src/index.js` — `db.init()`-এর পরে `billing.startExpiryScanJob()` কল করা হয়েছে
  (এক লাইন সংযোজন, security config-এ কোনো হাত দেওয়া হয়নি)।
- `server/src/routes/platform.js` — নতুন রুট:
  - `GET/POST /institutions/:id/payments` — পেমেন্ট-ইতিহাস দেখা/রেকর্ড করা
  - `POST /billing/expiry-scan` — শিডিউলের বাইরে ম্যানুয়ালি একবার স্ক্যান চালানো
  - `GET /migrations/tenants` — কোন কোন tenant schema আছে তার তালিকা (মাইগ্রেশন চালানোর আগে
    প্রিভিউ)
  - `POST /migrations/run` — দেওয়া SQL সব tenant schema-তে চালানো
- `server/scripts/registry-cli.js` — নতুন কমান্ড: `record-payment`, `payments`, `expiry-scan`,
  `migrate-tenants <sql-file>`।
- `server/public-platform/app.js`, `styles.css` — প্রতিটা প্রতিষ্ঠানের সারিতে "পেমেন্ট" বাটন
  (রেকর্ড + ইতিহাস দেখার মোডাল), টুলবারে "মেয়াদ স্ক্যান চালান" ও "মাইগ্রেশন টুল" বাটন যোগ হয়েছে।
- `.env.example` — `BILLING_AUTOSUSPEND_INTERVAL_MINUTES`, `DISABLE_BILLING_AUTOSUSPEND`
  ডকুমেন্টেড।

### কেন এভাবে

- **পেমেন্ট লেজার ম্যানুয়াল, কিন্তু এক্সটেনসিবল** — `reference` কলামে গেটওয়ের নিজস্ব
  ট্রানজেকশন আইডি রাখা হয়, যাতে পরে মিলিয়ে দেখা (reconciliation) সহজ হয়। ফাংশনটার সিগনেচার
  এমনভাবে রাখা হয়েছে যাতে ভবিষ্যতে সত্যিকারের গেটওয়ে-webhook একই ফাংশন ব্যবহার করতে পারে।
- **আগে থেকেই বেশি মেয়াদ থাকলে সেটা নষ্ট হয় না** — `recordPayment` বর্তমান
  `subscription_ends_at`/`trial_ends_at` যেটা এখনো ভবিষ্যতে আছে, তার উপর নতুন দিন যোগ করে,
  "এখন থেকে" গোনা শুরু করে না।
- **প্রতিটা tenant-এর মাইগ্রেশন আলাদা ট্রানজ্যাকশনে** — ৪০টা প্রতিষ্ঠানের মধ্যে ১৪ নম্বরটায়
  কোনো অপ্রত্যাশিত ড্রিফটের কারণে ব্যর্থ হলে ১-১৩ নম্বরের সফল পরিবর্তন থেকে যায়, ১৫-৪০ নম্বরও
  সুযোগ পায় — একটার ব্যর্থতায় বাকি সবার সফল মাইগ্রেশন রোলব্যাক হয়ে যায় না। ব্যর্থ হওয়া অংশ
  পরে আলাদাভাবে আবার চেষ্টা করা যায়।
- **অটো-সাসপেন্ড `setInterval` দিয়ে, নতুন npm প্যাকেজ ছাড়া** — এই কাজের জন্য পূর্ণাঙ্গ cron
  এক্সপ্রেশন পার্সার লাগে না, নির্দিষ্ট বিরতিতে একটা সহজ sweep-ই যথেষ্ট। কোনো প্রতিষ্ঠান এখনো
  মেয়াদ পার না করলে প্রতিটা sweep শুধু একটা সস্তা SELECT — তাই এটা সবসময় চালু রাখা নিরাপদ,
  এমনকি এখনো কোনো tenant provision না হওয়া ডিপ্লয়মেন্টেও।
- **মাইগ্রেশন রুট SQL টেক্সট নিজেই লগ করে না** — অডিট-লগে শুধু SQL-এর দৈর্ঘ্য আর সফল/ব্যর্থ
  সংখ্যা রাখা হয়, পুরো SQL না — মাইগ্রেশন SQL অনেক লম্বা হতে পারে, আর এটা নিজে বারবার দেখার
  মতো স্পর্শকাতর তথ্য না।

### সীমাবদ্ধতা (যা এই ধাপে করা হয়নি)

- **কোনো লাইভ পেমেন্ট গেটওয়ে ইন্টিগ্রেশন নেই** — উপরের "স্কোপ সংক্রান্ত সিদ্ধান্ত" দেখুন। bKash/
  Nagad/SSLCommerz-এর মার্চেন্ট অ্যাকাউন্ট রেজিস্ট্রেশন ও ক্রেডেনশিয়াল হাতে পেলে একটা
  webhook route যোগ করে `registryDb.recordPayment()` কল করানো তুলনামূলক ছোট কাজ।
- **মাইগ্রেশন টুল idempotent SQL ধরে নেয়** — এটা নিজে থেকে কোনো SQL "নিরাপদ কিনা" যাচাই করে
  না (যেমন `DROP TABLE`ও চালিয়ে দেবে যদি আপনি সেটা পেস্ট করেন)। এটা ইচ্ছাকৃত — অপারেটরকে
  ঠিক কী চালানো হচ্ছে তা নিজে পড়ে/লিখে দিতে বাধ্য করা, কোনো "শেষ schema ফাইল চালাও" শর্টকাট
  রাখা হয়নি।
- Sandbox পরিবেশে network/npm না থাকায় `npm run check` সরাসরি চালানো যায়নি — শুধু
  `node --check`-এ প্রতিটা `.js` ফাইল সিনট্যাক্স-ভ্যালিডেট করা হয়েছে। পুশের আগে নিজের মেশিনে
  একবার `npm run check` চালিয়ে নিন।

### টেস্ট করতে

```bash
# পেমেন্ট রেকর্ড করুন (এতে subscription_ends_at বেড়ে যাবে ও status active হয়ে যাবে)
...
# মেয়াদ শেষ হওয়া প্রতিষ্ঠান খুঁজে সাসপেন্ড করুন (এটা সার্ভার চালু থাকলে এমনিতেই প্রতি ঘণ্টায় চলে)
node server/scripts/registry-cli.js expiry-scan

# একটা টেস্ট মাইগ্রেশন ফাইল বানিয়ে সব tenant schema-তে চালান
echo "ALTER TABLE students ADD COLUMN IF NOT EXISTS test_migration_col text;" > /tmp/test-migration.sql
node server/scripts/registry-cli.js migrate-tenants /tmp/test-migration.sql
```

Super-Admin প্যানেলে (`/platform`) প্রতিটা প্রতিষ্ঠানের সারিতে এখন "পেমেন্ট" বাটন আছে (রেকর্ড +
ইতিহাস), টুলবারে "মেয়াদ স্ক্যান চালান" (ম্যানুয়ালি একবার) ও "মাইগ্রেশন টুল" (SQL বক্স + ফলাফল
টেবিল) বাটন আছে।

---

## ৬টি ভাগই সম্পন্ন — সংক্ষিপ্ত সারাংশ

এখন এই কোডবেস দিয়ে:
- একই সার্ভার/ডিপ্লয়মেন্ট দিয়ে একাধিক মাদ্রাসাকে সার্ভিস দেওয়া যায় (`MULTI_TENANT_MODE=true`),
  প্রতিটির ডেটা সম্পূর্ণ আলাদা schema-তে থাকে।
- নতুন প্রতিষ্ঠান যোগ, সাসপেন্ড/সক্রিয়করণ, সাবস্ক্রিপশন-পরিবর্তন, পেমেন্ট রেকর্ড, মেয়াদ শেষে
  অটো-সাসপেন্ড, আর সব প্রতিষ্ঠানে একসাথে schema-migration — সবকিছু `/platform` প্যানেল বা CLI
  থেকে টার্মিনাল ছাড়াই (প্যানেল থেকে) করা যায়।
- `MULTI_TENANT_MODE` বন্ধ রাখলে (ডিফল্ট) বর্তমান single-tenant ডিপ্লয়মেন্ট অবিকল আগের মতোই
  চলে — এই পুরো ৬-ভাগের কোনো কিছুই বাধ্যতামূলক নয়, প্রয়োজন হলে চালু করার মতো একটা সুইচ।
