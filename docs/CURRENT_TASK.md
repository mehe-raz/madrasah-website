...
## Status: DONE (Phase 1–4 সম্পূর্ণ — নিচে Phase 4A/4B-এর বিস্তারিত)
...
- [x] **Phase 4 (4A + 4B) সম্পন্ন — Phase 4 ও পুরো টাস্ক সম্পূর্ণ শেষ** —
  ইন-অ্যাপ সেটআপ গাইড পেজ, আলাদা রুটে (আলাদা lazy-loaded পেজ, বড় কনটেন্ট
  বলে মূল `AttendanceDevices.tsx`-এর সাথে একই ফাইলে বান্ডেল না করে —
  AGENTS.md-এর কোড-স্প্লিটিং প্যাটার্ন অনুসরণ করে)। নতুন
  `client/src/modules/AttendanceDeviceGuide.tsx` — ৪টা নাম্বার-করা ধাপ-কার্ড
  (১. ডিভাইস যোগ করুন, ২. hardware-bridge দিয়ে কানেক্ট করুন, ৩. ছাত্র
  এনরোলমেন্ট — "স্ক্যান করে বসান" রেফারেন্স করে, ৪. কিয়স্ক স্ক্রিন খোলা —
  `window.location.origin` দিয়ে আসল কিয়স্ক URL প্যাটার্ন দেখায়), প্রতি ধাপে
  lucide আইকন, শেষে একটা সামারি কার্ড। **Phase 4B-এর ভিজ্যুয়াল সিদ্ধান্ত:**
  প্ল্যান ডকের ধারা ৩-এ দেওয়া দুইটা অপশনের মধ্যে **icon-ভিত্তিক ধাপ-ডায়াগ্রাম**
  বেছে নেওয়া হয়েছে (কোনো নতুন asset/screenshot লাগে না, sandbox-এ যা সম্ভব) —
  এটা শুরু করার আগে আলাদাভাবে কনফার্ম করা হয়নি, কাজ এগিয়ে নেওয়ার জন্য
  যুক্তিসঙ্গত ডিফল্ট হিসেবে ধরে নেওয়া হয়েছে। ব্যবহারকারী চাইলে পরে আসল
  স্ক্রিনশট যোগ করা যাবে (structure পরিবর্তন ছাড়াই, প্রতিটা `Step` কম্পোনেন্টে
  একটা ছোট ইমেজ-স্লট যোগ করে)। **Step 2 (bridge কানেক্ট)-এর একটা সততার নোট:**
  মূল প্ল্যানের ভাষা ছিল "সরাসরি URL কনফিগার (bridge ছাড়া) বনাম bridge চালানো" —
  কিন্তু কোডবেসে bridge-ছাড়া-সরাসরি-কানেক্ট পথ (কীবোর্ড-এমুলেশন কার্ড
  রিডারের জন্য, `ATTENDANCE_DEVICE_PLAN.md`-এর section ৩-এ শুধু সম্ভাবনা
  হিসেবে উল্লেখ ছিল) এখনো তৈরি হয়নি — তাই গাইডে এটাকে বাস্তবসম্মতভাবে
  "এই সহজ পদ্ধতি এখনো যোগ করা হয়নি, দরকার হলে জানাবেন" হিসেবে লেখা হয়েছে,
  একটা ready-to-use দ্বিতীয় পথ হিসেবে না দেখিয়ে। বাকি সব ফাইল: `App.tsx`-এ
  নতুন lazy রুট `attendance-devices/guide` (বিদ্যমান "attendance"
  permission-এর আওতাতেই, আলাদা permission bucket লাগেনি); `AttendanceDevices.tsx`-এ
  পেজ-হেডার এখন `.page-header` (title+subtitle বাম, নতুন "কীভাবে কানেক্ট
  করব?" বাটন ডানে, `useNavigate` দিয়ে); `lib/icons.ts`-এ দুইটা নতুন আইকন
  (`bridge`: Usb, `kiosk`: Tv — কোনো নতুন dependency না, lucide-react
  আগে থেকেই ইনস্টলড); `index.css`-এ নতুন `.device-guide__*` স্কোপড ক্লাস
  (কোনো raw `style={{}}` ব্যবহার হয়নি, AGENTS.md ডিজাইন-সিস্টেম রুল মেনে);
  `i18n/bn.ts`/`en.ts`-এ নতুন `attendanceDeviceGuide` ব্লক (১৫টা কী, key-parity
  স্ক্রিপ্ট দিয়ে যাচাই করা হয়েছে) + `attendanceDevices` ব্লকে একটা নতুন
  `guideLink` কী। ব্রেস-ব্যালান্স স্ক্রিপ্ট দিয়ে প্রতিটা এডিট/নতুন `.tsx`/`.ts`
  ফাইল যাচাই করা হয়েছে — sandbox-এ নেটওয়ার্ক বন্ধ থাকায় `npm run check`
  নিজে চালানো যায়নি। **এই ডেলিভারির CMD-তে `npm run check` চালিয়ে পাস করলে
  তবেই commit/push হবে** — কোনো lint/typecheck এরর এলে সেটাই এই টাস্কের
  শেষ বাকি অংশ। `npm run check` পাস করার পর ম্যানুয়ালি গাইড পেজটা একবার
  চোখে দেখে নেওয়া ভালো (Phase 1-3-এর আসল UI-এর সাথে বর্ণনা মিলছে কিনা)।

### বাকি
কোনো কোড বাকি নেই। শুধু: (১) ব্যবহারকারীর মেশিনে `npm run check` পাস করা
কনফার্ম করা (ফেইল করলে ফিক্স করা লাগবে — উপরের নোট দেখুন), (২) নতুন গাইড
পেজ (`/attendance-devices/guide`) ব্রাউজারে একবার ম্যানুয়ালি দেখে নেওয়া।
