...
ট্র্যাকিং, "পারা" কলাম ইত্যাদি) না দেখে, কিন্তু ছাত্র ভর্তি/হিসাব-
নিকাশ/অ্যাটেন্ডেন্স/রেজাল্ট ইত্যাদি কোর ফিচার আগের মতোই পাবে।

সম্পূর্ণ কোডবেস (client + server + DB schema) লাইন বাই লাইন দেখে গবেষণা করা
হয়েছে — বিস্তারিত আবিষ্কার, ৮টা Phase-এর ব্রেকডাউন, স্কোপের বাইরে কী, এবং
৪টা "খোলা প্রশ্ন" (ডিফল্ট প্রস্তাবসহ) `docs/GENERAL_MODE_PLAN.md`-এ আছে।

সংক্ষেপে মূল আবিষ্কার: classTree/রোল/সাবজেক্ট/মার্কেটিং-পেজ ইতিমধ্যেই
জেনারিক (কোনো পরিবর্তন লাগবে না); আসল কাজ কেন্দ্রীভূত (১) হিফজ ট্র্যাকিং
ফিচার-মডিউলের গেটিং, (২) `student.dept` এনাম, (৩) `para` কলাম, (৪)
`registry.institutions`-এ `institution_type` কলাম নেই — এই ৪ জায়গায়।

### সম্পন্ন
- [x] সম্পূর্ণ কোডবেস গবেষণা + `docs/GENERAL_MODE_PLAN.md` লেখা হয়েছে,
  ব্যবহারকারীকে চ্যাটে সারসংক্ষেপ জানানো হয়েছে।
- [x] **Phase 1 (DB স্কিমা)** — `server/sql/registry_schema.sql`-এ
  (protected path — AGENTS.md রুল ৪ অনুযায়ী, প্ল্যান ডকে আগেই ফ্ল্যাগ করা
  ছিল বলে সরাসরি এগোনো হয়েছে) ফাইলের শেষে যোগ হয়েছে:
  - `alter table registry.institutions add column if not exists
    institution_type text not null default 'madrasah'` — নতুন কলাম,
    ডিফল্ট `'madrasah'` (খোলা প্রশ্ন ২-এর প্রস্তাব অনুযায়ী), বিদ্যমান
    টেন্যান্টের আচরণ অপরিবর্তিত থাকে।
  - `institutions_institution_type_check` কনস্ট্রেইন্ট —
    `institution_type in ('madrasah', 'general')` (খোলা প্রশ্ন ১-এর
    দুই-ভ্যালু প্রস্তাব অনুযায়ী)।
  - এই মাইগ্রেশন এখনো কোথাও read হচ্ছে না (`registryDb.js`,
    `createInstitution` ইত্যাদি স্পর্শ করা হয়নি) — শুধু কলাম আছে, কোনো
    visible effect নেই। Phase 2 থেকে এটা read/ব্যবহার শুরু হবে।
  - **single-tenant রিমাইন্ডার:** `MULTI_TENANT_MODE=false` ডিপ্লয়মেন্টে
    `registry.institutions`-এ কোনো row-ই থাকে না, তাই এই কলাম সেখানে
    প্রযোজ্য না — Phase 2-এ ওই কেসের জন্য আলাদা fallback (settings-টেবিল
    key, classOptionsTree-র প্যাটার্নে) লাগবে, প্ল্যান ডকে নোট করা আছে।
  - **মাল্টি-টেন্যান্ট রিমাইন্ডার:** আগে থেকে provision করা যেকোনো
    টেন্যান্টের জন্য এই `alter table` স্টেটমেন্ট Super-Admin-এর "run SQL
...
- [x] **Phase 6 (i18n টেক্সট ক্লিনআপ) সম্পন্ন** — §২.৩-এ তালিকাভুক্ত সবগুলো
  key-এর টেক্সট-ভ্যালু জেনেরিক করা হয়েছে (key নাম অপরিবর্তিত রাখা
  হয়েছে — যেমন `madrasaInfo` — কারণ key rename করলে Settings.tsx/
  Students.tsx-এর সব ব্যবহারস্থলও বদলাতে হতো, প্ল্যানের "খুব ছোট কাজ"
  স্কোপের বাইরে চলে যেত; শুধু ইউজার-facing টেক্সট বদলানো হয়েছে):
  - `client/src/i18n/bn.ts` — `settings.madrasaInfo` ("মাদ্রাসার তথ্য"
    → "প্রতিষ্ঠানের তথ্য"), `settings.name` ("মাদ্রাসার নাম" →
    "প্রতিষ্ঠানের নাম"), `students.madrasaInfo` ("মাদ্রাসা তথ্য" →
    "প্রতিষ্ঠানের তথ্য", `settings.madrasaInfo`-র সাথে এখন সামঞ্জস্যপূর্ণ),
    `pricing.subtitle` ("আপনার মাদরাসার প্রয়োজন..." → "আপনার
    প্রতিষ্ঠানের প্রয়োজন..."), `websiteContent.departmentsSummary`
    ("হিফজ, নাজেরা, কিতাব, জেনারেলসহ..." থেকে নির্দিষ্ট বিভাগের নাম বাদ
    দিয়ে "প্রতিষ্ঠানের বিভাগ ও প্রোগ্রামসমূহ।")।
  - `client/src/i18n/en.ts` — একই ৫টা key-র ইংরেজি ভার্সন
    ("Madrasa Information" → "Institution Information" ইত্যাদি), প্লাস
    `settings.restoreHelp`-এর "madrasah backup" → "backup" (bn.ts-এর
    এই key-টা যাচাই করে দেখা গেছে আগে থেকেই জেনেরিক ছিল, শুধু en.ts-এ
    "madrasah" শব্দটা ছিল — bn.ts স্পর্শ করা হয়নি এই key-তে)।
  - **যাচাই:** উভয় ফাইলেই "মাদ্রাসা"/"মাদরাসা"/"madrasa" (কেস-ইনসেনসিটিভ)
    গ্রেপ করে নিশ্চিত করা হয়েছে টেক্সট-ভ্যালুতে আর কিছু বাকি নেই (শুধু
    `madrasaInfo` key-নাম দুটোতে "madrasa" শব্দ থেকে গেছে, যা ইচ্ছাকৃত —
    উপরে ব্যাখ্যা করা হয়েছে)। bracket-balance স্ক্রিপ্টে দুটো ফাইলই
    ব্যালান্সড পাওয়া গেছে। `client/src/pages/Home.tsx`-এর ফলব্যাক টেক্সট
    ইচ্ছাকৃতভাবে স্পর্শ করা হয়নি — সেটা আলাদাভাবে Phase 7-এ ধরা আছে
    প্ল্যানে।

- [x] **Phase 7 (মার্কেটিং/হোমপেজ ফলব্যাক টেক্সট) সম্পন্ন** — খোলা প্রশ্ন
  ৪-এর প্ল্যান-প্রস্তাবিত ডিফল্ট (টাইপ-নির্ভর ডাইনামিক) অনুসরণ করা
  হয়েছে, আলাদা কনফার্মেশনের অপেক্ষা না করে (আগের Phase-গুলোর মতোই
  প্ল্যানের প্রস্তাব সরাসরি গ্রহণ করা হয়েছে)। মূল বুলেটে শুধু
  `Home.tsx`-এর `site.name || "মাদ্রাসা"` লাইনের কথা ছিল, কিন্তু
  ইমপ্লিমেন্ট করতে গিয়ে দেখা গেল `institution_type` পাবলিক API-তেই
  এক্সপোজড ছিল না (এটা `settings` টেবিলের কী না, `registry.institutions`
  টেবিলের কলাম — সম্পূর্ণ ভিন্ন schema), তাই সেটা প্রথমে যোগ করতে হয়েছে:
  - `server/src/lib/publicSettings.js` — `tenantContext.get()` থেকে
    (Phase 5-এর মতোই) `institution_type` পড়ে নতুন `institutionType`
    ফিল্ড হিসেবে `/api/public/settings`-এর রেসপন্সে যোগ করা হয়েছে,
    ফলব্যাক `"madrasah"` (কোনো tenant context না থাকলে — single-tenant
    ডিপ্লয়মেন্ট)।
  - `client/src/types/index.ts` — `PublicSettings` ইন্টারফেসে
    `institutionType: string` যোগ।
  - `client/src/lib/publicSiteDefaults.ts` — `FALLBACK_SETTINGS`-এ
    `institutionType: "madrasah"` যোগ (টাইপ স্যাটিসফাই করতে); এর
    `name: "মাদ্রাসা"` কে জেনেরিক `"প্রতিষ্ঠান"`-এ বদলানো হয়েছে — এটা
    শুধু সম্পূর্ণ API আউটেজে (অফলাইন) ব্যবহার হয়, তখন কোনো tenant ডেটাই
    (institutionType সহ) পাওয়া যায় না বলে টাইপ-নির্ভর করার উপায় নেই,
    তাই এখানে সবসময়-জেনেরিক অপশনটাই একমাত্র সঠিক পছন্দ।
  - `client/src/pages/Home.tsx` — মূল ফিক্স: `fallbackName = site.name
    || (site.institutionType === "general" ? "প্রতিষ্ঠান" : "মাদ্রাসা")`।
  - `server/src/lib/seoMeta.js` — একই সমস্যা সার্ভার-সাইড SEO
    মেটা-ট্যাগেও ছিল (ক্রলার/লিংক-প্রিভিউ বটদের জন্য `<title>`/OG
    ট্যাগ) — হার্ডকোড `DEFAULT_NAME` কনস্ট্যান্টকে `defaultName(site)`
    ফাংশনে বদলে একই institutionType-চেক প্রয়োগ করা হয়েছে
    `PUBLIC_ROUTES`-এর সবগুলো বিল্ডারে (২১টা ব্যবহারস্থল, সবগুলোতেই
    `site` প্যারামিটার স্কোপে ছিল বলে নিরাপদে বদলানো গেছে)। এটা মূল
    বুলেটে উল্লেখ ছিল না কিন্তু ঠিক একই "homepage fallback name" সমস্যা,
    ঠিক একই `site` অবজেক্ট ব্যবহার করে (যেটা এই Phase-এই enrich করা
    হলো), তাই সামঞ্জস্যপূর্ণভাবে একসাথে ফিক্স করা হয়েছে।
  - **ইচ্ছাকৃতভাবে স্পর্শ করা হয়নি (ফ্ল্যাগ করা হলো):**
    `client/src/lib/printReport.ts:361`-এর `settings.name || "মাদ্রাসা"`
    — এটা অথেন্টিকেটেড ইউজারের ভর্তি-ফর্ম প্রিন্ট ইউটিলিটি, পাবলিক
    মার্কেটিং/হোমপেজ না (Phase 7-এর টাইটেলের স্কোপের বাইরে)। `Home.tsx`-এর
    `defaultBadges = ["হিফজ", "নাজেরা", "কিতাব", "জেনারেল"]` (ট্যাব-বিভাগ
    ফলব্যাক, তেন্যান্ট নিজে ক্লাস/কোর্স কন্টেন্ট কাস্টমাইজ না করলে দেখা
    যায়) — একই থিমের কিন্তু ভিন্ন, বড় পরিসরের কাজ, Phase 7-এর মূল
    বুলেটে নেই।
  - bracket-balance স্ক্রিপ্ট দিয়ে ৫টা পরিবর্তিত ফাইলই যাচাই করা হয়েছে
    (সব ব্যালান্সড), এবং প্রতিটা `PublicSettings`/`getPublicSettings`
    কনজিউমার (client + server) খুঁজে নিশ্চিত করা হয়েছে নতুন required
    ফিল্ডের কারণে কিছু ভাঙেনি।
  - **সংশোধন (২০২৬-০৮-২০, ব্যবহারকারীর `npm run check` রান থেকে ধরা
    পড়েছে):** উপরের consumer-চেক `PublicSettings\s*=\s*{`-এর মতো একটা
    regex প্যাটার্নে সীমাবদ্ধ ছিল বলে `client/src/pages/WebsitePreview.tsx`-এর
    `setSite({...})` কলটা মিস হয়ে গিয়েছিল (`useState<PublicSettings>`
    থেকে টাইপ ইনফার হয়, কোনো এক্সপ্লিসিট `PublicSettings` অ্যানোটেশন
    ওই লাইনে ছিল না বলে grep ধরতে পারেনি) — `tsc -b --noEmit` একটা
    `TS2345` এরর দিয়েছিল (`institutionType` মিসিং)। ফিক্স: এই পেজ
    অথেন্টিকেটেড `api.getSettings()` কল করে (public API না — ফাইলের
    টপ কমেন্টে এটাই ইচ্ছাকৃত বলে লেখা আছে), যেটাতে `institution_type`
    এক্সপোজড না, তাই `institutionType: FALLBACK_SETTINGS.institutionType`
    ("madrasah") ডিফল্ট বসানো হয়েছে — এই পেজে institutionType-এর
    একমাত্র প্রভাব `Home.tsx`-এর ফলব্যাক *নাম*-এ, তাও শুধু
    `site.name` খালি থাকলে (বিরল, কসমেটিক এজ-কেস একটা প্রিভিউ পেজে)।
    **শিক্ষা:** ভবিষ্যতে "সব consumer খুঁজে বের করা হয়েছে" claim করার
    আগে `tsc`/`npm run check` আসলে চালিয়ে (বা অন্তত grep-এর সীমাবদ্ধতা
...
অনুমোদিত ভ্যালু, বিদ্যমান টেন্যান্টের ডিফল্ট, হিফজ override অপশন, Home.tsx
ফলব্যাক লজিক)। ব্যবহারকারী নির্দিষ্ট উত্তর না দিলে সেই প্ল্যানের ডিফল্ট
প্রস্তাবগুলো ধরেই Phase 1 থেকে এগোনো হবে (এই প্রজেক্টের অন্য প্ল্যান
ডকগুলোর কনভেনশন অনুযায়ী)।

এই টাস্কটা বিদ্যমান IN_PROGRESS শিফট/সিডিউল টাস্কের থেকে সম্পূর্ণ আলাদা —
নিচের এন্ট্রি (শিফট/সিডিউল) স্পর্শ করা হয়নি, "শুরু কর" বললে ব্যবহারকারীকে
স্পষ্ট করতে হবে কোন টাস্ক (এই ডকুমেন্টের §"How to use this file" রুল
অনুযায়ী, নতুন টাস্ক আলাদা এন্ট্রি থাকবে, মার্জ হবে না)।
...
ডিভাইস-পাঞ্চ এলে `lib/devicePunch.js` সবসময় `status = 'উপস্থিত'` হার্ডকোড
করে বসায় (ছাত্র ও স্টাফ দুটোতেই), কোনো সময়ের সাথে তুলনা ছাড়াই।

দেখুন `docs/SHIFT_SCHEDULE_PLAN.md` — সম্পূর্ণ প্রেক্ষাপট (ডিভাইস-পাঞ্চ
পাইপলাইন স্টাফদের জন্যও ইতিমধ্যে বাস্তবায়িত থাকার আবিষ্কারসহ — সেই প্ল্যানের
§৭ দেখুন), ডিজাইন সিদ্ধান্ত (শিফট Admin তৈরি করবে, ক্লাস/জামাত-লেভেলে বরাদ্দ,
প্রতি ছাত্র আলাদা না), নতুন DB স্কিমা (`shifts`, `class_shifts` টেবিল +
`staff.shiftId` + `attendance`/`staff_attendance`-এ `entryTime`/`exitTime`
কলাম), ৯টা Phase-এর বিস্তারিত ব্রেকডাউন, স্কোপের বাইরে কী, এবং ৪টা "খোলা
প্রশ্ন" (ডিফল্ট প্রস্তাবসহ) যা কাজ শুরুর আগে ব্যবহারকারীর সাথে কনফার্ম করা
উচিত।
...
সাথে এই টাস্কের সম্পর্ক নেই, আলাদা এন্ট্রি হিসেবেই থাকবে, মার্জ করা হয়নি
(AGENTS.md-এর "brand-new task while IN_PROGRESS" নিয়ম অনুযায়ী)।

---

## Task: স্টাফ/শিক্ষক এন্ট্রি + স্টাফ অ্যাটেন্ডেন্স (৭ Phase-এ, পুরো পরিকল্পনা
`docs/STAFF_ATTENDANCE_PLAN.md`-এ)

## Status: IN_PROGRESS (কোনো Phase এখনো শুরু হয়নি — এই সেশনে শুধু প্ল্যান
লেখা হয়েছে)

Started: 2026-08-16 (প্ল্যান লেখার তারিখ)

বর্তমান `users`/Settings-এর "ব্যবহারকারী" সেকশন শুধুই সফটওয়্যার লগইন
অ্যাকাউন্ট (নাম/ইমেইল/পাসওয়ার্ড/রোল) — প্রকৃত স্টাফ/শিক্ষকের তথ্য (ফোন,
পদবি, যোগদানের তারিখ) রাখার কোনো জায়গা নেই, এবং যেসব স্টাফের লগইন লাগবে
না (পিয়ন/দারোয়ান ইত্যাদি) তাদের সিস্টেমে ঢোকানোর কোনো উপায় নেই। এছাড়া
`attendance` টেবিল/`Attendance.tsx` সম্পূর্ণ শিক্ষার্থী-কেন্দ্রিক — স্টাফ
অ্যাটেন্ডেন্স রেকর্ড করার কোনো টেবিল/রুট/পেজ নেই।

দেখুন `docs/STAFF_ATTENDANCE_PLAN.md` — সম্পূর্ণ প্রেক্ষাপট, ডিজাইন
সিদ্ধান্ত (`users`-লগইন আর নতুন `staff`-কর্মী-তালিকা আলাদা টেবিল, ঐচ্ছিক
`userId` লিংক দিয়ে সংযুক্ত), নতুন DB স্কিমা (২ টেবিল: `staff`,
`staff_attendance` — দ্বিতীয়টা বিদ্যমান `attendance` টেবিলের হুবহু
প্যাটার্নে), ৭টা Phase-এর বিস্তারিত ব্রেকডাউন (ফাইল/এন্ডপয়েন্ট/UI-সেকশন
নাম করে), যা যা আগে থেকেই আছে ও রি-ইউজ হবে তার তালিকা, স্কোপের বাইরে কী
(বেতন ম্যানেজমেন্ট, ডিভাইস-ভিত্তিক স্টাফ পাঞ্চ, অ্যাটেন্ডেন্স রিপোর্ট —
সবই এই টাস্কের বাইরে), এবং শেষে ৪টা "খোলা প্রশ্ন" (ডিফল্ট প্রস্তাবসহ) যা
কাজ শুরুর আগে ব্যবহারকারীর সাথে কনফার্ম করা উচিত (Hostel Manager-কে
staffAttendance পারমিশন দেওয়া হবে কিনা, স্টাফ হার্ড-ডিলিট নাকি শুধু
Active/Inactive টগল, "ছুটি" আলাদা স্ট্যাটাস লাগবে কিনা, ছবি-আপলোড এখনই
নাকি পরে)।

### সম্পন্ন
- [x] পূর্ণাঙ্গ প্ল্যান ডকুমেন্ট লেখা হয়েছে, ব্যবহারকারীকে চ্যাটে ডিজাইন
  সিদ্ধান্ত ও খোলা প্রশ্নগুলো জানানো হয়েছে।
...
- [ ] Phase 2–7 — বিস্তারিত `docs/STAFF_ATTENDANCE_PLAN.md`-এ।

### নোট
এই মুহূর্তে অন্য কোনো IN_PROGRESS টাস্ক (নিচে "নিজের ফোন/SIM দিয়ে বাল্ক
SMS") আছে — সেটা এই টাস্কের সাথে সম্পর্কহীন, আলাদা এন্ট্রি হিসেবেই থাকবে,
মার্জ করা হয়নি (AGENTS.md-এর "brand-new task while IN_PROGRESS" নিয়ম
অনুযায়ী)।
...
ভিত্তিক পেইড SMS সিস্টেম [`routes/sms.js`/`smsSender.js`/`sms_wallets`]-এর
সাথে **সম্পূর্ণ আলাদা, সমান্তরাল** সিস্টেম — কোনো ওয়ালেট/ডিডাকশন নেই, খরচ
প্রতিষ্ঠানের নিজের ফোনের SIM ব্যালেন্সে; ভয়েস-কল-ব্রডকাস্ট এই টাস্কের বাইরে,
প্ল্যান ফাইলে কারণ লেখা আছে)

## Status: IN_PROGRESS (কোনো Phase এখনো শুরু হয়নি — এই সেশনে শুধু প্ল্যান
লেখা হয়েছে, ব্যবহারকারীর সাথে আলোচনা করে কনফার্ম করা হয়েছে)

Started: 2026-08-13 (প্ল্যান লেখার তারিখ)

দেখুন `docs/OWN_SIM_BULK_SMS_GATEWAY_PLAN.md` — সম্পূর্ণ প্রেক্ষাপট, বেছে
নেওয়া প্রযুক্তি (SMSGate, sms-gate.app, Cloud mode, ওপেন-সোর্স/ফ্রি — কেন
বেছে নেওয়া হলো তার যুক্তিসহ), নতুন DB স্কিমা (৩ টেবিল:
`own_sms_gateway`/`sms_contacts`/`sms_broadcast_logs`), ৭টা Phase-এর
বিস্তারিত ব্রেকডাউন (প্রতিটা ফাইল/এন্ডপয়েন্ট/UI-সেকশন নাম করে), ও শেষে
৫টা "গুরুত্বপূর্ণ নকশা-সিদ্ধান্ত" যেগুলো পরের কোনো এজেন্ট প্রশ্ন না করে
অনুমান করে বদলে ফেলবে না এমন স্পষ্টভাবে লেখা আছে (যেমন: বিদ্যমান SMS
ওয়ালেট সিস্টেমের সাথে না মেশানো, students টেবিলের সাথে সম্পর্কহীন রাখা,
`{নাম}`+`{name}` দুটো প্লেসহোল্ডারই সাপোর্ট করা)।
...
- [x] **Phase 4 সম্পন্ন (2026-08-13)** — ফ্রন্টএন্ড গেটওয়ে-কানেক্ট UI,
  প্ল্যান ফাইলের ধারা অনুযায়ী:
  - `client/src/types/index.ts` — নতুন `OwnSmsGatewayStatus` টাইপ
    (`PaymentGatewayStatus`-এর শেপে)।
  - `client/src/lib/api.ts` — নতুন `getOwnSmsGatewayStatus`/
    `connectOwnSmsGateway`/`disconnectOwnSmsGateway` (Phase 2-এর
    `/api/own-sms-gateway/*` রুট তিনটার client wiring)।
  - নতুন `client/src/modules/BulkSmsGateway.tsx` —
    `PaymentGatewaySettings.tsx`-এর (bKash self-connect) ভিজ্যুয়াল
    প্যাটার্নে status card (কানেক্টেড/না, শেষ চেক করা সময়, এরর) +
    ৫-ধাপের বাংলা/ইংরেজি নির্দেশনা কার্ড (`AttendanceDeviceGuide.tsx`-এর
    `device-guide__step` ক্লাস reuse করে) + কানেক্ট ফর্ম
    (username/password) + ডিসকানেক্ট বাটন। শুধু `components/ui/`
    (`Card`/`Field`/`Input`/`Button`) + বিদ্যমান ক্লাস — কোনো নতুন raw
    `style={{}}` নেই।
  - `client/src/i18n/bn.ts` + `en.ts` — নতুন `bulkSms` ব্লক (৩০টা কী,
    key-parity স্ক্রিপ্ট দিয়ে যাচাই করা হয়েছে)।
  - অ্যাপের ইনস্টল/Cloud-mode-চালু করার ধাপগুলো এই সেশনে
    `docs.sms-gate.app`/`sms-gate.app`/GitHub থেকে ওয়েব সার্চ করে
    আবার যাচাই করা হয়েছে (Cloud Server টগল অন করে \"Online\" বাটনে
    ট্যাপ করলে Cloud Server সেকশনে username/password দেখায় — প্ল্যানের
    আগের অনুমানের সাথে মূলত মিলে যায়, ধাপগুলো সাধারণভাবে লেখা হয়েছে
    যাতে অ্যাপের সামান্য UI পরিবর্তনেও ভুল না হয়)। Play Store-এর সরাসরি
    ডাউনলোড লিংক ইচ্ছাকৃতভাবে হার্ডকোড করা হয়নি (লিংক বদলে যেতে পারে) —
    নির্দেশনায় শুধু অ্যাপের নাম (\"SMS Gateway for Android\") বলা আছে,
    Play Store-এ সার্চ করে বের করা যাবে এমনভাবে লেখা।
  - **এই কম্পোনেন্ট এখনো কোনো রুট/নেভে ওয়্যার করা হয়নি** — প্ল্যানের
    Phase 6-এ চূড়ান্ত মডিউল-গঠন সিদ্ধান্ত (এই ফাইলই আলাদা থাকবে, নাকি
    কন্টাক্ট/কম্পোজ সেকশনসহ একটাই `BulkSms.tsx`-এ মেশানো হবে) অনুযায়ী
    `App.tsx`/`Sidebar.tsx`-এ যোগ হবে — এখন যোগ করলে Phase 6-এ আবার
    সরাতে হতো।
...
- [x] **Phase 6 সম্পন্ন (2026-08-13)** — ফ্রন্টএন্ড কম্পোজ/সেন্ড UI +
  রাউট/নেভ/i18n ওয়্যারিং, প্ল্যান ফাইলের মডিউল-গঠন সিদ্ধান্ত অনুযায়ী:
  - নতুন `client/src/modules/BulkSms.tsx` — একটাই মডিউল, ৩টা ট্যাব
    (গেটওয়ে/কন্টাক্ট/কম্পোজ ও পাঠান)। প্রথম দুই ট্যাব সরাসরি Phase
    4/5-এর `BulkSmsGateway`/`SmsContactsManager` কম্পোনেন্ট import করে
    বসায় (নতুন করে লেখা হয়নি)। তৃতীয় ট্যাবে নতুন `ComposeSection`:
    মেসেজ `<Textarea>` (`{নাম}` হিন্টসহ), নির্বাচিত প্রথম কন্টাক্টের
    নাম দিয়ে লাইভ প্রিভিউ (`personalize()` হেল্পার —
    `server/src/routes/sms.js`-এর একই `{নাম}`/`{name}` রিপ্লেস-লজিক
    ক্লায়েন্ট-সাইডে মিরর করা, শুধু প্রিভিউ দেখানোর জন্য, আসল পাঠানো
    এখনো সার্ভারেই হয়), প্রাপক checkbox-লিস্ট + মাস্টার "সবাইকে
    নির্বাচন করুন" চেকবক্স (indeterminate state সহ), পাঠানোর আগে
    `confirm()` (কতজনকে যাচ্ছে জানিয়ে), পাঠানোর পর সামারি ব্যানার
    (মোট/সফল/ব্যর্থ)। গেটওয়ে ডিসকানেক্টেড থাকলে পাঠান বাটন disabled +
    সতর্কতা ব্যানার।
  - `client/src/lib/api.ts` — নতুন `sendSmsBroadcast()`
    (`POST /sms/broadcast`, Phase 3-এর বিদ্যমান ব্যাকএন্ড এন্ডপয়েন্টের
    client wiring — এই ফাংশনটাই আগের কোনো ফেজে বাদ পড়ে গিয়েছিল)।
  - `client/src/types/index.ts` — নতুন `SmsBroadcastResult` টাইপ
    (`{total, sent, failed}`, সার্ভারের রেসপন্স শেপে)।
  - `client/src/lib/icons.ts` — নতুন `bulkSms: Send` আইকন-কী (বিদ্যমান
    `sms` কী থেকে আলাদা, যেহেতু এটা একটা সম্পূর্ণ ভিন্ন নেভ-আইটেম/রুট)।
  - `client/src/App.tsx` — `BulkSms` lazy-imported, নতুন `bulk-sms` রুট
    (বিদ্যমান `sms`/`payment-gateway` রুটের ঠিক প্যাটার্নে,
    `PlanFeatureGate feature="sms"` — একই "sms" প্ল্যান-ফিচার রিইউজ,
    প্ল্যান ফাইলের সিদ্ধান্ত অনুযায়ী নতুন ফিচার-কী লাগেনি)।
  - `client/src/components/Sidebar.tsx` — NAV_IDS-এর বাইরে নতুন
    `NavLink` (SMS/bKash ব্লকের একই প্যাটার্নে — "settings" পারমিশন +
    "sms" প্ল্যান-লক, নিজস্ব লেবেল/আইকন)।
  - `client/src/i18n/bn.ts` + `en.ts` — `nav.bulkSms` +
    বিদ্যমান `bulkSms` ব্লকেই ১৯টা নতুন `tab*`/`compose*` কী যোগ
    (key-parity স্ক্রিপ্ট দিয়ে যাচাই করা হয়েছে, দুই ফাইলেই মিলেছে)।
  - `client/src/index.css` — নতুন `.bulk-sms-tabs`, `.sms-recipient-list`/
    `.sms-recipient-row` (কম্পোজ ট্যাবের প্রাপক-checkbox-লিস্ট, Phase
    5-এর `.sms-contact-row` থেকে ইচ্ছাকৃতভাবে আলাদা রাখা হয়েছে —
    সেটা ৪-কলাম গ্রিড, এখানে checkbox+নাম+নম্বর ৩-কলাম), এবং
    `.alert--sky` (প্রিভিউ লাইনের জন্য, বিদ্যমান rose/amber/emerald
    alert ভ্যারিয়েন্টের পাশে নতুন neutral ভ্যারিয়েন্ট)।
  - বাংলাদেশি লোকাল ফোন নম্বর (`sms_contacts.phone`, `01XXXXXXXXX`) →
    আন্তর্জাতিক ফরম্যাট রূপান্তর সার্ভার-সাইডে (`toInternational()`,
    Phase 3-এই করা) হয়, তাই এই ফ্রন্টএন্ড কোনো ফরম্যাট রূপান্তর করে না
    — শুধু কন্টাক্টের `phone` যেমন সেভ করা আছে তেমনই দেখায়।
  - bracket-balance script + i18n key-parity script দিয়ে যাচাই করা
    হয়েছে (সব মিলেছে)। **পুরো `npm run check` প্যাকেজড CMD-তেই প্রথম
    যাচাই হবে** (sandbox-এ network/node_modules না থাকায় এখানে চালানো
    যায়নি, আগের সব Phase-এর প্যাটার্নে)।

### বাকি (পরের এজেন্ট এখান থেকে চালিয়ে যাবে)
- [ ] Phase 7 — পলিশ/এজ-কেস, তারপর এই এন্ট্রি DONE-এ পাল্টানো (`npm run
  check` পাস করলে)।
- [x] **প্যাকেজিং বাগ ফিক্স (2026-08-13, পরের সেশনে ধরা পড়েছে)** —
  আগের ডেলিভারির zip-এ Phase 6-এর ফাইলগুলো (এই `CURRENT_TASK.md`
  আপডেট + `client/src/App.tsx`-এর `/bulk-sms` রুট +
  `client/src/components/Sidebar.tsx`-এর নেভ আইটেম +
  `client/src/modules/BulkSms.tsx` + `types/index.ts`/`lib/api.ts`/
  `lib/icons.ts`/`i18n/bn.ts`/`i18n/en.ts`/`index.css`-এর যোগ হওয়া
  অংশ) ভুলভাবে root প্রজেক্টের ভেতরে একটা নেস্টেড
  `madrasah-website/` সাবফোল্ডারে জিপ হয়ে গিয়েছিল — root-এর আসল
  ফাইলগুলোতে merge হয়নি। ফলে ব্যবহারকারী `npm run check` চালিয়ে
  pass পেলেও (কারণ সেই সময় root ফাইলগুলো এখনো Phase 5-এর অবস্থায়
  স্থিতিশীল ছিল, ভাঙা কিছু ছিল না) বাস্তবে সাইটের সাইডবারে "বাল্ক SMS"
  মেনু আইটেম দেখাচ্ছিল না। এই সেশনে নেস্টেড ফোল্ডারের প্রতিটা ডিফ
  আলাদাভাবে root-এর সঠিক ফাইলে বসানো হয়েছে (bracket-balance +
  i18n key-parity স্ক্রিপ্ট দিয়ে পুনরায় যাচাই করা হয়েছে, সব মিলেছে),
  নেস্টেড `madrasah-website/` ফোল্ডারটা এই ডেলিভারির zip-এ **নেই**
  (শুধু root-এর সঠিক ফাইলগুলো)। **শিক্ষা:** ভবিষ্যতে zip প্যাকেজ
  করার আগে `find . -maxdepth 1 -type d` দিয়ে root-এ কোনো অপ্রত্যাশিত
  ডুপ্লিকেট প্রজেক্ট-সাবফোল্ডার (client/server/docs আবার ভেতরে) নেই
  তা যাচাই করা উচিত — `npm run check` পাস করলেই ফাইল ঠিক জায়গায়
  আছে তার গ্যারান্টি না, কারণ পুরনো/অব্যবহৃত ফাইল ভাঙা কোড ছাড়াই
  থেকে যেতে পারে।
- [ ] উপরের DB migration নোট (own_sms_gateway/sms_contacts/
  sms_broadcast_logs তিনটা টেবিল) — single-tenant হলে সার্ভার
  রিস্টার্টেই হয়ে যাবে; multi-tenant হলে Super-Admin migration
  টুল দিয়ে ম্যানুয়ালি রান করতে হবে (এখনো করা হয়নি কিনা ব্যবহারকারীর
  কাছে নিশ্চিত হওয়া হয়নি)।

### নোট
প্রতিটা Phase শেষ হলে এখানে "সম্পন্ন"-এ সরিয়ে "বাকি" থেকে বাদ দিতে হবে,
প্যাকেজিং/zip-এর আগে (এই ফাইলের নিচের "How to use this file" নির্দেশনা
অনুযায়ী)। বিস্তারিত implementation নোট এখানে না লিখে
`OWN_SIM_BULK_SMS_GATEWAY_PLAN.md`-এই আপডেট করা উচিত যদি কোনো সিদ্ধান্ত
বদলায়, যাতে একটাই সোর্স-অফ-ট্রুথ থাকে — এখানে শুধু ট্র্যাকিং/সামারি।
